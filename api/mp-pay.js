...
<body>
  <div class="box">
    ...
    <button id="payBtn" class="btn">Calcular y continuar</button>
  ...
  <script>
    (function () {
      ...
      let preparedOrderRef = "";
      let preparedEurAmount = null;
      ...
      async function preparePaymentData(orderRef, eurAmount) {
        ...
        preparedOrderRef = orderRef;
        preparedEurAmount = eur;
        preparedMxnAmount = mxnAmount;

        setStatus("Revisa la conversión y haz clic en “Continuar a Mercado Pago”.");
      ...
      payBtn.addEventListener("click", async function () {
        try {
          const currentOrder = orderRefEl.value.trim();
          const currentEur = normalizeNumber(amountEurEl.value.trim());

          if (
            preparedMxnAmount !== null &&
            preparedOrderRef === currentOrder &&
            preparedEurAmount === currentEur
          ) {
            await createPreferenceAndRedirect(currentOrder, preparedMxnAmount);
            return;
          }

          await preparePaymentData(currentOrder, amountEurEl.value.trim());
        } catch (err) {
          console.error(err);
      ...
      if (INITIAL_ORDER_REF && INITIAL_AMOUNT_EUR) {
        preparePaymentData(INITIAL_ORDER_REF, INITIAL_AMOUNT_EUR);
      } else {
